     ...
     & iostat=readerror)
      ...
      ! modavpout
      read(1,*,iostat=readerror) dum(1:5), dtmax
      if (readerror.ne.0) then 
        write(*,*) 'parameter read error'
        stop
      end if
      ...
      ! number of cells between walls
      read(1,*,iostat=readerror) dum(1:4), dpdz
      if (readerror.ne.0) then 
        write(*,*) 'parameter read error'
        stop
      end if
      if (dpdz>0.d0) then
        write(*,*) 'choose a negative pressure gradient'
        stop
      end if 
      write(*,*) 'dpdz', dpdz
      ...
      close(unit=1)
      accel=-dpdz/sum(mass*npartdens*mconv)
      write(*,*) 'acceleration', accel
